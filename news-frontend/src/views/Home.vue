            ...
            <div class="card-body">
              ...
              <div class="d-flex justify-content-between align-items-center">
                <div class="btn-group" style="margin: 8px 0;">
                  ...
                  <button class="btn btn-sm btn-danger" v-on:click="showMsgBox(news._id, news.title)"><b>Delete post</b></button>
                    <div class="mb-2">
                    <!-- <button class="btn btn-sm btn-danger" @click=>Delete post</button> -->
                    </div>
                </div>
              </div>
...
export default {
  ...
  methods: {  
    ...
    deleteNews(id) {
      axios.delete(`${server.baseURL}/news/delete?newsID=${id}`).then(() => {
        this.fetchNews()
      });
    },
    showMsgBox(id, title) {
          this.$bvModal.msgBoxConfirm(`Please confirm that you want to delete this "${title}" post.`, {
          ...
          cancelTitle: 'No, I changed my mind',
          footerClass: 'p-2',
          hideHeaderClose: false,
          centered: true
        })
        .then(value => {
            value ? this.deleteNews(id) : false
          })
    }
  }
};
